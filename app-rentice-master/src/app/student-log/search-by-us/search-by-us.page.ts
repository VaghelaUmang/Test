...
@Component({
  selector: 'app-search-by-us',
  templateUrl: './search-by-us.page.html',
  styleUrls: ['./search-by-us.page.scss'],
})
export class SearchByUsPage implements OnInit {
  ...
  doSummaryExport(bUnitOnly, flag = false) {
    if(this.exportSummaryList.length < 1) {
      ...
      setTimeout(() => {
          this.pdf.autoTable({html: '#my-table11',
            columnStyles: {
          ...
          let pdfName = "Qualification Summary_" + new Date().getTime() + ".pdf";
  ...
  loadQualData() {
    ...
    this.courseList = [];
    const url = "https://www.artisanideas.co.nz/api/app-rentice/get_qualification.php?faculty=" + this.user.faculty;
  ...
  selectAll(item) {
    ...
    this.router.navigateByUrl('/search-by-us-update');
  }
