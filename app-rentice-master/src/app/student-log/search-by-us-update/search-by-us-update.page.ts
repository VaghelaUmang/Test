...
@Component({
  selector: 'app-search-by-us-update',
  templateUrl: './search-by-us-update.page.html',
  styleUrls: ['./search-by-us-update.page.scss'],
})
export class SearchByUsUpdatePage implements OnInit {
  ...
  ionViewWillEnter() {
    try {
      if(!window.device) {
        window.ga('send', 'pageview', 'Student Log Search us Update Page');
      } else {
        if(this.ga) {
          this.ga.trackView('Student Log Search us Update Page');
        }
      }
    } catch(e) {
      console.log(e)
