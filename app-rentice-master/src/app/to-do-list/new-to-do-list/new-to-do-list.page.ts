...
@Component({
  ...
  templateUrl: './new-to-do-list.page.html',
  styleUrls: ['./new-to-do-list.page.scss'],
})
export class NewToDoListPage implements OnInit {
  ...
  ngOnInit() {
	...
	ionViewWillEnter() {
  	...
  	console.log(this.user);
		this.todolistCnt = localStorage.getItem("todolistCnt");
		this.data = {
        list_no: parseInt(this.todolistCnt) + 1,
        user_id: '',
        listname: '',
        project_no: '',
  ...
  takePicture(sourceType) {
	...
	async saveData() {
      ...
      const list_no = this.todolistCnt + 1;
      ...
      let data = {
          ...
          list_no: list_no.toString(),
          image: this.imgURL
      };
      
      const url = "https://www.artisanideas.co.nz/api/app-rentice/add_todolist.php";
      const loading = await this.loadingController.create({
	      message: '',
	    });
	    await loading.present();
      this.http.post(url, data).subscribe((resp: any) => {
          ...
          this.alertController.create({
			      ...
			      message: this.translate.instant('Todo list has been created.'),
			      buttons: [{
			          text: 'Okay',
			          handler: () => {
