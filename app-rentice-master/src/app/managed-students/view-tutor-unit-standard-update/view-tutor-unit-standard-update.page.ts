...
@Component({
  selector: 'app-view-tutor-unit-standard-update',
  templateUrl: './view-tutor-unit-standard-update.page.html',
  styleUrls: ['./view-tutor-unit-standard-update.page.scss'],
})
export class ViewTutorUnitStandardUpdatePage implements OnInit {
  ...
  ionViewWillEnter() {
    try {
      if(!window.device) {
        window.ga('send', 'pageview', 'Tutor Student Log Search us Update Page');
      } else {
        if(this.ga) {
          this.ga.trackView('Tutor Student Log Search us Update Page');
        }
      }
    } catch(e) {
      console.log(e)
