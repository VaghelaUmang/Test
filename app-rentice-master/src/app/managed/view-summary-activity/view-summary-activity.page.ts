...
@Component({
  selector: 'app-staff-view-summary-activity',
  templateUrl: './view-summary-activity.page.html',
  styleUrls: ['./view-summary-activity.page.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class StaffViewSummaryActivityPage implements OnInit {
  ...
  ngOnInit() {
	...
	ionViewDidEnter() {
    this.subCat = JSON.parse(localStorage.getItem('staffSubSummary'));
	...
	goItem(item) {
    localStorage.setItem('staffsummaryActivity', JSON.stringify(item));
    this.router.navigateByUrl('/staff-view-summary-detail')
  }
  ...
  exportPDF() {
    ...
    let pdfName = "Staff_Activity_Summary_" + new Date().getTime() + ".pdf";
    let yPos = 90;
