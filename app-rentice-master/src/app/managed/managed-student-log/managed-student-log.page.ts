...
@Component({
    selector: 'app-managed-student-log',
    templateUrl: './managed-student-log.page.html',
    styleUrls: ['./managed-student-log.page.scss'],
})
export class ManagedStudentLogPage implements OnInit {
    ...
    ionViewWillEnter() {
        console.log("==managed student log page===");
    ...
    doExport(flag = false) {
        if (this.exportList.length < 1) {
            ...
            setTimeout(() => {
              ...
              let pdfName = "staff_row" + new Date().getTime() + ".pdf";
        ...
        this.pdf.text('Activity1: ' + pdfItem.inventory_item, 10, 30)
        this.pdf.text('Activity2: ' + pdfItem.inventory_item2, 10, 40)
        this.pdf.text('Activity3: ' + pdfItem.inventory_item3, 10, 50)
        this.pdf.text('Project: ' + pdfItem.address, 10, 60)
        this.pdf.text('Hours: ' + pdfItem.inventory_value, 10, 70)
        this.pdf.text('Notes: ' + pdfItem.notes, 10, 80)
        this.pdf.text('Employer Notes: ' + pdfItem.boss_notes, 10, 95)
        this.pdf.text('Tutor Notes: ' + pdfItem.tutor_notes, 10, 105)
        this.pdf.text('Date: ' + pdfItem.inventory_purchased, 10, 115)
        */
    ...
    doSummaryExport(flag = false) {
        if(this.exportSummaryList.length < 1) {
          ...
          setTimeout(() => {
              this.pdf.autoTable({html: '#my-table14',
                columnStyles: {
              ...
              let pdfName = "staff_row_summary_" + new Date().getTime() + ".pdf";
              if(this.global.isCordova) {
                let pdfOutput = this.pdf.output();
                this.global.exportPDFCordova(pdfOutput, pdfName);
              } else {
    ...
    async loadRowStatus() {
      ...
      this.http.get(url).subscribe((resp: any) => {
          if(resp.error == true) {
            console.log("there is an error while get row status");
            return ;
          }
    ...
    async changeMainCategory() {
        ...
        const url = "https://www.artisanideas.co.nz/api/app-rentice/get_subcategory_by_main.php?main_cat=" + this.data.main_category + "&faculty=" + this.user.faculty;
        const loading = await this.loadingController.create({
            message: '',
        });
        await loading.present();
        this.http.get(url).subscribe((resp: any) => {
            ...
            loading.dismiss();
            this.global.displayAlert("There is an error when get subcategory");
        });
    }
    ...
    goStudentItem(slidingItem: IonItemSliding, item) {
        if(slidingItem)
          slidingItem.close();
        localStorage.setItem("currentManagedStudent", JSON.stringify(item));
        console.log(item);
        this.router.navigateByUrl('/view-managed-student');
    }
    ...
    async viewSummary() {
      ...
      this.global.staffMaincategoryList = this.mainCategoryList;
      this.global.staffSubcategoryList = this.subCategoryList;
      this.global.staffActivityList = this.activityList;
      this.global.staffInventoryList = this.AllInventoryList;
    ...
    goSubSummary(item) {
      console.log(item);
      localStorage.setItem('staffMainSummary', JSON.stringify(item));
      this.router.navigateByUrl('/staff-view-summary-sub')
    }
}
