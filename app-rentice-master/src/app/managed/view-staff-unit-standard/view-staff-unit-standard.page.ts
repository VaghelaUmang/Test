...
@Component({
  selector: 'app-view-staff-unit-standard',
  templateUrl: './view-staff-unit-standard.page.html',
  styleUrls: ['./view-staff-unit-standard.page.scss'],
})
export class ViewStaffUnitStandardPage implements OnInit {
  ...
  doSummaryExport(bUnitOnly, flag = false) {
    if(this.exportSummaryList.length < 1) {
      ...
      setTimeout(() => {
          this.pdf.autoTable({html: '#my-table10',
            columnStyles: {
          ...
          let pdfName = "trainee_summary_" + new Date().getTime() + ".pdf";
    ...
    try {
      if(!window.device) {
        window.ga('send', 'pageview', 'Managed Staff Record of Work Unit Standard Page');
      } else {
        if(this.ga) {
          this.ga.trackView('Managed Staff Record of Work Unit Standard Page');
        }
      }
    } catch(e) {
      console.log(e)
    ...
    this.http.get(url).subscribe((resp: any) => {
        ...
        this.signedOffList = resp.data.filter(item => {
          return (parseInt(item.provider) == parseInt(this.user.provider) && 
            ...
            parseInt(item.student) == parseInt(this.user.tutor))
        })
  ...
  changeType() {
    ...
    this.unitList.forEach(item => {
      if(this.data.type == '') {
        ...
        if(item.course_code == this.user.qual && item.theory == type) {
          this.unitRenderList.push(item)
        }
      }
    })
  ...
  selectAll(item) {
    ...
    this.router.navigateByUrl('/view-staff-unit-standard-update');
  }
